...
public class BotEC extends Bot {
    ...
    public static void turn() throws GameActionException {
        ...
        // TODO: First capped ECs should spawn muckrakers only for 15 turns for safety
        if (!targetFound) rc.setFlag(0);
        flag = rc.getFlag(id);
        if (rc.isReady()) {
            if (rc.getRoundNum() >= 700) {
                if (robotsSpawned % 2 == 0) {
                    ...
                    trySpawn(RobotType.SLANDERER, Math.min(randomInfluence, influence));
                } else {
                    int randomInfluence;
                ...
                // Spawn pattern S P M P M (w/ max 80 slanderers per base)
