...
public class BotMuckraker extends Bot {
    ...
    public static void setFlag(RobotInfo[] nearby) throws GameActionException {
        ...
        for (RobotInfo robot : nearby) {
			...
			if (robotType.equals(RobotType.MUCKRAKER) && robotTeam.equals(us)) {
				int allyFlag = rc.getFlag(robot.ID);
				if (allyFlag == 0) continue;
				MapLocation to = Comm.getLocationFromFlag(allyFlag);
				if (otherFlag == -1) { 
					otherFlag = allyFlag;
				} else if (here.distanceSquaredTo(to) < here.distanceSquaredTo(Comm.getLocationFromFlag(otherFlag))) {
					otherFlag = allyFlag;
				}
			}	
	...
	private static Direction lastDir = Direction.NORTH; 
