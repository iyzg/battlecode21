...
public class BotEC extends Bot {
    ...
    private static ECBehavior behavior = ECBehavior.ECO;
    ...
    private static void spawnEco() throws GameActionException {
		if (influence >= 20) {
            trySpawn(RobotType.SLANDERER, 949, 949, 0);
		} else {
			trySpawn(RobotType.MUCKRAKER, 1, 1, 0); 
		}
    }

    private static void spawnPoliBuffer() throws GameActionException {
		trySpawn(RobotType.POLITICIAN, 15, 25, 10);
    }
	...
	private static int needed = -1, spawned = 0;
	private static int slanderersSpawned = 0;
    public static void turn() throws GameActionException {
		...
		if (enemyFound && behavior == ECBehavior.ECO) {
			behavior = ECBehavior.POLI_BUFFER;
			needed = (int)(1.5 * slanderersSpawned);
		} else if (spawned >= needed && behavior == ECBehavior.POLI_BUFFER) {
            behavior = ECBehavior.GREEDY;
		} else if (robotsSpawned == 0 && rc.getRoundNum() >= 200) {
            behavior = ECBehavior.BUFFER;
        } else if (robotsSpawned == 0 && rc.getRoundNum() >= 10) {
            behavior = ECBehavior.GREEDY;
