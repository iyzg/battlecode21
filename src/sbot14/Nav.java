...
public class Nav extends Bot {
	...
	private static boolean tryMoveDirect() throws GameActionException {
        ...
        for (int dist = 1; dist * dist <= rc.getType().sensorRadiusSquared; ++dist) {
            for (int a = 0; a <= dist; ++a) {
                int b = dist - a;
                MapLocation loc = here.translate(a, b);
                if (rc.canSenseLocation(loc) && here.distanceSquaredTo(dest) > loc.distanceSquaredTo(dest) && rc.sensePassability(loc) > maxPassability) {
					tempDest = loc;
					maxPassability = rc.sensePassability(loc);
				}
		...
		Direction toDest = here.directionTo(tempDest);
		maxPassability = -1;
		System.out.println("Temp Dest @ " + tempDest);
