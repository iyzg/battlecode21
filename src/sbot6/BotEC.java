...
public class BotEC extends Bot {
    ...
    // TODO: Only send friendly if you converted it
    ...
    private static void updateFlag() throws GameActionException {
        ...
        for (int id : scouts) {
            if (id != 0 && rc.canGetFlag(id)) {
                ...
                int bucket = Comm.getExtraInformationFromFlag(allyFlag);
                ...
                for (int i = 0; i < tail; ++i) {
                    if (ECs[i].equals(ecLoc)) {
                        buckets[i] = bucket;
                        if (friendly != converted[i]) fresh[i] = true;
        ...
        if (tail != 0) {
            ...
            for (int i = 0; i < 12; ++i) {
                if (fresh[i] && converted[i]) {
                    Comm.sendLocation(ECs[i], 0, 2);
                    freshFound = true;
                    break;
                }
            }
            ...
            if (!converted[cur]) {
                ...
                if (!ECs[cur].equals(here) && !freshFound) {
                    Comm.sendLocation(ECs[cur], 0, 2);
                    cur = (cur + 1) % tail;
                }
            }
        }
    ...
    private static void spawnSafe() throws GameActionException {
        ...
        if (robotsSpawned % 5 == 0 || robotsSpawned % 5 == 3) {
            ...
            // TODO: If thicc income, start making medium polis
        }
    }
    ...
    public static void turn() throws GameActionException {
        ...
        updateFlag();
        // TODO: Be able to set target if you spot EC within range :D
        
        if (robotsSpawned == 0 && rc.getRoundNum() >= 200) {
