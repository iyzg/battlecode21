...
public class BotMuckraker extends Bot {
    ...
    public static void setFlag(RobotInfo[] nearby) throws GameActionException {
		...
		MapLocation proxyLoc = null;
		int proxyFlag = -1;
        for (RobotInfo robot : nearby) {
			Team robotTeam = robot.getTeam();
			RobotType robotType = robot.getType();
			MapLocation robotLocation = robot.location;
			if (robotTeam.equals(us) && robotType.equals(RobotType.MUCKRAKER)) {
				int robotFlag = rc.getFlag(robot.ID);
				if (robotFlag == 0) continue;

				MapLocation robotFlagLocation = Comm.getLocationFromFlag(robotFlag);
				if (proxyLoc == null) {
					proxyLoc = robotFlagLocation;
					proxyFlag = robotFlag;
				} else if (here.distanceSquaredTo(robotFlagLocation) < here.distanceSquaredTo(proxyLoc)) {
					proxyLoc = robotFlagLocation;
					proxyFlag = robotFlag;
				}
		...
		} else if (proxyLoc != null) {
			rc.setFlag(proxyFlag);
		} else rc.setFlag(0);
